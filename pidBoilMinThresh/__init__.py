...
@cbpi.controller
class PIDBoilMinThresh(KettleController):
 """
 The original pid boil logic with added min temp only above which the logic activates
 """"
    ...
    e_boil = Property.Number("Boil Threshold", True, 80,description="Temperature for Boil threshold. Full power mode!")
    f_start = Property.Number("Mash Threshold", True, 60,description="Temperature above which the logic activates")
    def stop(self):
        '''
    ...
    def run(self):
        ...
        while self.is_running():
            ...
            if self.get_target_temp() <= float(self.f_start) or self.get_target_temp() >= float(self.e_boil):
                self.heater_on(100)
                self.sleep(1)
            else:
